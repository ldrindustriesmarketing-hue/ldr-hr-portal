    ...
    return (
      <div className="min-h-screen bg-gray-50 p-6 flex items-center justify-center">
        <div className="bg-white p-8 rounded-lg shadow-lg text-center max-w-md">
          ...
          <p className="text-gray-600 mb-4">Your risk assessment has been created successfully.</p>
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white p-8 rounded-lg shadow">
          <h1 className="text-3xl font-bold mb-2" style={{ color: '#f89939' }}>Create Risk Assessment</h1>
          <p className="text-gray-600 mb-6">Build a new risk assessment form for employees</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g., General Workplace Safety Assessment"
                required
                disabled={loading}
              />
            </div>
            ...
            <div className="pt-6 border-t">
                  ...
                  {questions.map((question, index) => (
                    <div key={question.id} className="bg-white p-4 rounded-lg border border-gray-200 flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500 mt-1">Type: <span className="font-medium">{question.type.replace('-', ' ')}</span></p>
                      </div>
                      <button
                        type="button"
              ...
              {questions.length === 0 && (
                <div className="p-4 bg-blue-50 rounded-lg mb-6">
                  <p className="text-blue-700 text-sm">Add at least one question to create the assessment</p>
                </div>
              )}
            </div>
            ...
            <button
              ...
              style={{ backgroundColor: questions.length === 0 ? '#ccc' : '#f89939' }}
              className="w-full text-white py-3 rounded-lg font-semibold hover:opacity-90 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? 'Creating Assessment...' : `Create Risk Assessment (${questions.length} question${questions.length !== 1 ? 's' : ''})`}
            </button>
          </form>
        </div>
      </div>
